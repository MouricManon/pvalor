...
function SleepC({ data,date}: SleepProps) {
    ...
    return (<div id="sleeps">
         ...
         <ul >{data.length == 0?
            ...
            <> {data.map((i, index) => (
                <li key={index} id="onesleep"> Date : {date} <ul><li> Duration : {i.duration/60*0.001} </li><li> Efficiency : {i.efficiency}% </li><li> Minutes Asleep : {i.minutesAsleep}</li><li> Minutes Awake : {i.minutesAwake}</li><ul  className='notdone'><li> Stages :  </li><li>Deep sleep :</li><li> Light sleep : </li> <li>Rem sleep : </li></ul></ul></li>))}</>}
        </ul>
</div>)
}
